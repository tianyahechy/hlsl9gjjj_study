...
CRenderObject::~CRenderObject()
{
	...
	SAFE_RELEASE(m_pContantTableVS);
	SAFE_RELEASE(m_pConstantTablePS);
...
HRESULT CRenderObject::OnCreateDevice(IDirect3DDevice9 * pd3dDevice, TCHAR * FileName)
{
	...
	hr = D3DXLoadMeshFromX(_szModelFileName.c_str(), D3DXMESH_SYSTEMMEM, pd3dDevice, NULL, NULL, NULL, NULL, &pMeshSysMemClone);
	if (FAILED(hr))
	{
		return hr;
	}
	...
	/*
	//装入效果文件
	//szFile = L"D:\\lifei\\test\\hlsl高级精讲\\SimpleSample\\ResFile_1\\2_4_3_1.txt";
	LPD3DXBUFFER pCode = NULL;
	hr = D3DXCreateEffectFromFile(pd3dDevice, _szEffectFileName.c_str(), NULL, NULL, D3DXSHADER_DEBUG, NULL, &m_pEffect, &pCode);
	if (pCode)
	{
	...
	hr = D3DXCreateTextureFromFileEx(pd3dDevice, _szTextureFileName.c_str(), D3DX_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT,
	...
	*/
	...
	LPD3DXBUFFER pCode = NULL;
	LPD3DXBUFFER pCodeError = NULL;
	//装入顶点着色器
	//编译开关，设置成调试模式
	DWORD dwFlag = D3DXSHADER_DEBUG | D3DXSHADER_SKIPOPTIMIZATION;
	hr = D3DXCompileShaderFromFile(m_pVertexShaderFileName.c_str(), NULL, NULL, "RenderSceneVS", "vs_2_0", dwFlag, &pCode, &pCodeError, &m_pContantTableVS);
	if (FAILED(hr))
	{
		LPVOID pBuffer = pCodeError->GetBufferPointer();
		char* ErrInfo = (char*)pBuffer;
		pCodeError->Release();
		return hr;
	}
	//编译完成后，设置句柄
	hr = pd3dDevice->CreateVertexShader(reinterpret_cast<DWORD*> (pCode->GetBufferPointer()), &m_pVertexShader);
	if (FAILED(hr))
	{
		return E_FAIL;
	}
	//装入像素着色器
	//编译开关，设置成调试模式
	hr = D3DXCompileShaderFromFile(m_pPixelShaderFileName.c_str(), NULL, NULL, "RenderScenePS", "ps_2_0", dwFlag, &pCode, &pCodeError, &m_pConstantTablePS);
	if (FAILED(hr))
	{
		LPVOID pBuffer = pCodeError->GetBufferPointer();
		char* ErrInfo = (char*)pBuffer;
	...
	//编译完成后，设置句柄
	hr = pd3dDevice->CreatePixelShader(reinterpret_cast<DWORD*> (pCode->GetBufferPointer()), &m_pPixelShader);
	if (FAILED(hr))
	{
		return E_FAIL;
	}
...
HRESULT CRenderObject::OnResetDevice(IDirect3DDevice9 * pd3dDevice)
{
	...
	//取第三个参数的句柄
	D3DXHANDLE hANDLE = m_pEffect->GetParameter(NULL, 3);
	LPCSTR pStr;
	m_pEffect->GetString(hANDLE, &pStr);
	LPWSTR theStr = L"";
	//MultiByteToWideChar(CP_ACP, 0, pStr, -1, theStr, strlen(pStr));
	//取第二个参数的释义
	hANDLE = m_pEffect->GetParameter(NULL, 2);
	hANDLE = m_pEffect->GetAnnotation(hANDLE, 0);
	m_pEffect->GetString(hANDLE, &pStr);
	//MultiByteToWideChar(CP_ACP, 0, pStr, -1, theStr, strlen(pStr));
...
void CRenderObject::OnFrameMove(IDirect3DDevice9* pd3dDevice,D3DXMATRIXA16 * pWorld, D3DXMATRIXA16 * pView, D3DXMATRIXA16 * pProject, double fTime)
{
	...
	*/
	D3DXHANDLE matWorldHandle = m_pContantTableVS->GetConstantByName(0, "g_matWorld");
	m_pContantTableVS->SetMatrix(pd3dDevice, matWorldHandle, pWorld);
	D3DXHANDLE matViewHandle = m_pContantTableVS->GetConstantByName(0, "g_matView");
	m_pContantTableVS->SetMatrix(pd3dDevice, matViewHandle, pView);
	D3DXHANDLE matProjectHandle = m_pContantTableVS->GetConstantByName(0, "g_matProject");
	m_pContantTableVS->SetMatrix(pd3dDevice, matProjectHandle, pProject);
	D3DXHANDLE matTimeHandle = m_pContantTableVS->GetConstantByName(0, "g_fTime");
	m_pContantTableVS->SetFloat(pd3dDevice, matTimeHandle, fTime);
...
void CRenderObject::OnFrameRender(IDirect3DDevice9 * pd3dDevice)
{
	...
	*/
	...
	//设置纹理
	HRESULT hr = pd3dDevice->SetTexture(0, m_pColorTexture);
	//设置顶点着色器
...
void CRenderObject::setFiles(std::wstring szEffectFileName, std::wstring szModelFileName, std::wstring szTextureFileName)
{
	_szEffectFileName = szEffectFileName;
	_szModelFileName = szModelFileName;
	_szTextureFileName = szTextureFileName;
}

void CRenderObject::setVertexShaderFile(std::wstring strVertexFileName)
{
	m_pVertexShaderFileName = strVertexFileName;
}

void CRenderObject::setPixelShaderFile(std::wstring strPixelFileName)
{
	m_pPixelShaderFileName = strPixelFileName;
}

void CRenderObject::setTextureName(std::wstring strTextureName)
{
	_szTextureFileName = strTextureName;
}
