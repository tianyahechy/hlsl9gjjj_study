...
//绘制的物体
CRenderObjectDS				g_RenderObject_Ground;
CBoard						g_DebugTextureBoard;
CFullWindow					g_FullWindow;
                                     ...
                                     void* pUserContext )
{
	...
	SetCurrentDirectory(szRcPath);
	hr = g_FullWindow.OnCreateDevice(pd3dDevice );
	if (FAILED(hr))
	{
		return hr;
	}
	...
	SetCurrentDirectory(szRcPath);
	hr = g_DebugTextureBoard.OnCreateDevice(pd3dDevice);
	if (FAILED(hr))
	{
		return hr;
	}
                                    ...
                                    const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
   ...
   // g_Camera.SetProjParams( D3DX_PI / 4, fAspectRatio, 0.1f, 1000.0f );
	...
	g_DebugTextureBoard.OnResetDevice(pd3dDevice, pBackBufferSurfaceDesc);
...
void CALLBACK OnD3D9FrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
	...
	DXUT_BeginPerfEvent(D3DCOLOR_XRGB(0, 255, 0), L"RenderToTexture");
	{
    ...
    {
		DXUT_BeginPerfEvent(D3DCOLOR_XRGB(0, 255, 0), L"DebugTexture");
		{
			g_DebugTextureBoard.OnFrameRender(pd3dDevice, g_RenderObject_Ground.returnTexture(), 0);
		}
		DXUT_EndPerfEvent();

		DXUT_BeginPerfEvent(D3DCOLOR_XRGB(0, 255, 0), L"SSAO");
		{
			g_FullWindow.OnFrameRender(pd3dDevice, g_RenderObject_Ground.returnTexture());
		}
		DXUT_EndPerfEvent();
