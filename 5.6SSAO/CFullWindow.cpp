...
HRESULT CFullWindow::OnResetDevice(IDirect3DDevice9 * pd3dDevice)
{
	...
	D3DXHANDLE colorTextureHandle = m_pEffect->GetParameterByName(0, "g_texNoiseMap");
...
{
	D3DXHANDLE colorTextureHandle = m_pEffect->GetParameterByName(0, "g_texDepthTexture");
...
void CFullWindow::OnFrameRender(IDirect3DDevice9* pd3dDevice, D3DXMATRIXA16* pWorld, D3DXMATRIXA16* pView, D3DXMATRIXA16* pProject, double fTime)
{
	...
	hr = pd3dDevice->SetStreamSource(0, m_pVB, 0, sizeof(FULLWINDOW_D3DVERTEX));
	hr = pd3dDevice->SetIndices(m_pIB);
	D3DXHANDLE renderHandle = m_pEffect->GetTechniqueByName("techSSAO");
...
void CFullWindow::OnFrameRender(IDirect3DDevice9* pd3dDevice, LPDIRECT3DTEXTURE9 pDepthTexture, LPDIRECT3DTEXTURE9 pNormalTexture)
{
	D3DXHANDLE renderHandle = m_pEffect->GetTechniqueByName("techSSAO");
	m_pEffect->SetTechnique(renderHandle);

	D3DXHANDLE depthTextureHandle = m_pEffect->GetParameterByName(0, "g_texDepthTexture");
	m_pEffect->SetTexture(depthTextureHandle, pDepthTexture);
	D3DXHANDLE normalTextureHandle = m_pEffect->GetParameterByName(0, "g_texNormalTexture");
	m_pEffect->SetTexture(normalTextureHandle, pNormalTexture);
	D3DXHANDLE colorTextureHandle = m_pEffect->GetParameterByName(0, "g_texNoiseMap");
	m_pEffect->SetTexture(colorTextureHandle, m_pColorTexture);
