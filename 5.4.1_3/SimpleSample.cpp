...
void CALLBACK OnD3D9FrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
	...
	g_RenderObject_Ground.OnFrameMove(pd3dDevice, &g_matWorld, &g_matView, &g_matProject, fTime, &vViewPosition, &vViewPosition);
	//设置灯光位置给受光物体，同时给灯光物体
	...
	g_RenderLight.OnFrameMove(pd3dDevice, &g_matWorld, &g_matView, &g_matProject, fTime, &vViewPosition, &vLightLookAt);
