...
HRESULT CBoxObject::LoadResource(TCHAR * FileName)
{
	//打开文件
	DWORD uByte;
	//查看文件是否存在
...
//利用资源类型索引得到文件名
TCHAR * CBoxObject::GetResourceName(RESTYPE enuResType)
{
	TCHAR* szReturn = NULL;
...
void CBoxObject::Init()
{
}

HRESULT CBoxObject::OnCreateDevice(IDirect3DDevice9 * pd3dDevice, TCHAR * FileName)
{
	HRESULT hr = S_OK;
	...
	hr = D3DXCreateCubeTextureFromFile(pd3dDevice, szFile, &m_pCubeTexture);
	return hr;
}

HRESULT CBoxObject::OnResetDevice()
{
	HRESULT hr = S_OK;
	...
	D3DXHANDLE cubeTextureHandle = m_pEffect->GetParameterByName(0, "g_CubeTexture");
	hr = m_pEffect->SetTexture(cubeTextureHandle, m_pCubeTexture);

	return hr;
}

void CBoxObject::OnFrameMove(IDirect3DDevice9* pd3dDevice,D3DXMATRIXA16 * pWorld, D3DXMATRIXA16 * pView, D3DXMATRIXA16 * pProject, double fTime)
{
	...
	float Deternimant;
	D3DXMATRIX matOut;
	D3DXMatrixInverse(&matOut, &Deternimant, pWorld);
	D3DXMatrixTranspose(&matOut, &matOut);
	D3DXHANDLE matNormalInverseHandle = m_pEffect->GetParameterByName(0, "g_matWorldNormalInverse");
	m_pEffect->SetMatrix(matNormalInverseHandle, &matOut);

}

void CBoxObject::OnFrameRender(IDirect3DDevice9 * pd3dDevice)
{
	pd3dDevice->SetVertexDeclaration(m_pDecl);
	pd3dDevice->SetStreamSource(0, m_pVB, 0, sizeof(CBOXOBJECT_D3DVERTEX));
...
void CBoxObject::OnDestroyDevice()
{
	SAFE_RELEASE(m_pVB);
	SAFE_RELEASE(m_pIB);
	SAFE_RELEASE(m_pDecl);
