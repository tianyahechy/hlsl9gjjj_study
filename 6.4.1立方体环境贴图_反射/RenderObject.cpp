...
void CRenderObject::setViewVector(D3DXVECTOR3 * pVector3)
{
	...
	D3DXHANDLE viewPositionHandle = m_pEffect->GetParameterByName(0, "g_ViewPosition");
	m_pEffect->SetVector(viewPositionHandle, &v4Vector);
...
HRESULT CRenderObject::OnResetDevice()
{
	...
	hr = m_pEffect->SetTexture(colorTextureHandle, m_pCubeTexture);
