...
CRenderObject::CRenderObject()
{
	...
	m_pEffect = NULL;
	m_pColorTexture_1 = NULL;
	m_pColorTexture_2 = NULL;
	m_pColorTexture_3 = NULL;
...
CRenderObject::~CRenderObject()
{
	...
	SAFE_RELEASE(m_pColorTexture_1);
	SAFE_RELEASE(m_pColorTexture_2);
	SAFE_RELEASE(m_pColorTexture_3);
...
HRESULT CRenderObject::OnCreateDevice(IDirect3DDevice9 * pd3dDevice, TCHAR * FileName)
{
	...
	hr = D3DXCreateTextureFromFileEx(pd3dDevice, szFile, D3DX_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT,
		...
		NULL, NULL, &m_pColorTexture_1);
	if (FAILED(hr))
	{
		return hr;
	}

	szFile = GetResourceName(ENU_TEXNORMALMAP);
	hr = D3DXCreateTextureFromFileEx(pd3dDevice, szFile, D3DX_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT,
		0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 0,
		NULL, NULL, &m_pColorTexture_2);
	if (FAILED(hr))
	{
		return hr;
	}

	szFile = GetResourceName(ENU_SPECULARMAP);
	hr = D3DXCreateTextureFromFileEx(pd3dDevice, szFile, D3DX_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT,
		0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 0,
		NULL, NULL, &m_pColorTexture_3);
	if (FAILED(hr))
	{
		return hr;
	}
...
HRESULT CRenderObject::OnResetDevice()
{
	...
	hr = m_pEffect->SetTexture(colorTextureHandle, m_pColorTexture_1);
	if (FAILED(hr))
	{
		return hr;
	}
...
void CRenderObject::OnFrameMove(IDirect3DDevice9* pd3dDevice,D3DXMATRIXA16 * pWorld, D3DXMATRIXA16 * pView, D3DXMATRIXA16 * pProject, double fTime)
{
	...
	D3DXHANDLE colorTextureHandle1 = m_pEffect->GetParameterByName(0, "g_ColorTexture_1");
	m_pEffect->SetTexture(colorTextureHandle1, m_pColorTexture_1);
	D3DXHANDLE colorTextureHandle2 = m_pEffect->GetParameterByName(0, "g_ColorTexture_2");
	m_pEffect->SetTexture(colorTextureHandle2, m_pColorTexture_2);
	D3DXHANDLE colorTextureHandle3 = m_pEffect->GetParameterByName(0, "g_ColorTexture_3");
	m_pEffect->SetTexture(colorTextureHandle3, m_pColorTexture_3);
}
