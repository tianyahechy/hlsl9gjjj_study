...
void CRenderObject::setLightParament(D3DXVECTOR4 * vPosition, int iLight)
{

	D3DXHANDLE lightPositionHandle = NULL;
	switch (iLight)
	{
	case 1:
		lightPositionHandle = m_pEffect->GetParameterByName(0, "g_vLightPosition_1");
		m_pEffect->SetVector(lightPositionHandle, vPosition);
		break;
	case 2:
		lightPositionHandle = m_pEffect->GetParameterByName(0, "g_vLightPosition_2");
		m_pEffect->SetVector(lightPositionHandle, vPosition);
		break;
	default:
		break;
