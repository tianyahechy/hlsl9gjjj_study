...
void CRenderObject::setLightParamter(D3DXVECTOR4 * vPosition)
{
...
HRESULT CRenderObject::OnCreateDevice(IDirect3DDevice9 * pd3dDevice, TCHAR * FileName)
{
	...
	//高度纹理
	szFile = GetResourceName(ENU_TEXHEIGHTMAP);
	hr = D3DXCreateTextureFromFileEx(pd3dDevice, szFile, D3DX_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT,
		0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 0,
		NULL, NULL, &m_pHeightTexture);
	if (FAILED(hr))
	{
		return hr;
	}
...
HRESULT CRenderObject::OnResetDevice()
{
	...
	D3DXHANDLE heightTextureHandle = m_pEffect->GetParameterByName(0, "g_HeightTexture");
	hr = m_pEffect->SetTexture(normalTextureHandle, m_pHeightTexture);
