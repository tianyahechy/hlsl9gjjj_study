...
CRenderObject::~CRenderObject()
{
	...
	if (NULL != m_iColorGreen)
	{
		delete[] m_iColorGreen;
		m_iColorGreen = NULL;
	}

	if (NULL != m_iColorBlue)
	{
		delete[] m_iColorBlue;
		m_iColorBlue = NULL;
	}
...
HRESULT CRenderObject::OnCreateDevice(IDirect3DDevice9 * pd3dDevice, TCHAR * FileName)
{
	...
	//设置颜色
	//因为纹理有256*256大小
	int pixelNumber = 256 * 256;
	m_iColorRed = new int[pixelNumber];
	if (NULL == m_iColorRed )
	{
		return E_FAIL;
	}
	...
	{
		m_iColorRed[i] = 0x00ff0000;
	}

	m_iColorGreen = new int[pixelNumber / 4];
	if (NULL == m_iColorGreen)
	{
		return E_FAIL;
	}
	//初始化为绿色
	for (int i = 0; i < pixelNumber/4; i++)
	{
		m_iColorGreen[i] = 0x00ff0000;
	}

	m_iColorBlue = new int[pixelNumber / 16];
	if (NULL == m_iColorBlue)
	{
		return E_FAIL;
	}
	//初始化为蓝色
	for (int i = 0; i < pixelNumber / 16; i++)
	{
		m_iColorBlue[i] = 0x00ff0000;
	}
	...
	//锁住纹理，改变纹理
	{
		...
		//注意，只取第一层
		m_pColorTexture->GetLevelDesc(0, &desc);
		m_pColorTexture->LockRect(0, &lockRect, NULL, 0);
		CopyMemory(m_iColorRed, lockRect.pBits, pixelNumber * sizeof(m_iColorRed[0]));
		lockRect.Pitch = pixelNumber * sizeof(m_iColorRed[0]);
	...
	//通过页面操作纹理	
	{
		...
		m_pColorTexture->GetSurfaceLevel(0, &pSurface);
		if (NULL != pSurface)
		{
			D3DLOCKED_RECT lockRect;
			D3DSURFACE_DESC desc;
	...
	//测试取深度模板缓冲区
	{
		IDirect3DSurface9* pSurface = NULL;
		D3DLOCKED_RECT lockRect;
		pd3dDevice->GetDepthStencilSurface( &pSurface);
		pSurface->LockRect(&lockRect, NULL, D3DLOCK_NOSYSLOCK);
		pSurface->UnlockRect();
		pSurface->Release();
	}
	return hr;
