                                     ...
                                     void* pUserContext )
{
	...
	lstrcat(szRcPath, L"\\ResFile_1_SkyBox");
	SetCurrentDirectory(szRcPath);
	TCHAR * szFile_1 = L"ShmFile.shm";
	hr = g_SkyBox.OnCreateDevice(pd3dDevice, szFile_1);
	if (FAILED(hr))
	{
		return hr;
	}
	...
	lstrcpy(szRcPath, szAppPath);
	lstrcat(szRcPath, L"\\ResFile_2_ReflectWater");
	SetCurrentDirectory(szRcPath);
	TCHAR * szFile_2 = L"ShmFile.shm";
	hr = g_ReflectWater.OnCreateDevice(pd3dDevice, szFile_2);
	if (FAILED(hr))
	{
		return hr;
	}
                                    ...
                                    const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
   ...
   // g_Camera.SetProjParams( D3DX_PI / 4, fAspectRatio, 0.1f, 1000.0f );
	...
	g_ReflectWater.OnResetDevice();
	g_SkyBox.OnResetDevice();
...
void CALLBACK OnD3D9FrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
	...
	D3DXVECTOR3 vEyePt(20 * sin(fTime / 4.0), 3, 20 * cos(fTime / 4.0));
	D3DXVECTOR3 vLookAtPt(0, 0, 0);
	D3DXVECTOR3 vUpVec(0, 1, 0);
	D3DXMatrixLookAtLH(&g_matView, &vEyePt, &vLookAtPt, &vUpVec);

	g_ReflectWater.OnFrameMove(pd3dDevice, &g_matWorld, &g_matView, &g_matProject, fTime);
	g_ReflectWater.setViewVector(&vEyePt);
	g_SkyBox.OnFrameMove(pd3dDevice, &g_matWorld, &g_matView, &g_matProject, fTime);

    // Clear the render target and the zbuffer 
	pd3dDevice->Clear( 0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, D3DXCOLOR( 0, 0.25, 0.25, 0.0f ), 1.0f, 0 ) ;
	pd3dDevice->BeginScene();
	g_ReflectWater.OnFrameRender(pd3dDevice);
	g_SkyBox.OnFrameRender(pd3dDevice);
...
void CALLBACK OnD3D9DestroyDevice( void* pUserContext )
{
	...
	g_ReflectWater.OnDestroyDevice();
