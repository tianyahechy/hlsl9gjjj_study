...
void CRenderObject::setViewVector(D3DXVECTOR3 * pVector3)
{
	D3DXVECTOR4 v4Vector = D3DXVECTOR4(pVector3->x, pVector3->y, pVector3->z, 1.0);
	D3DXHANDLE vectorHandle = m_pEffect->GetParameterByName(0, "g_ViewPosition");
	m_pEffect->SetVector(vectorHandle, &v4Vector);
}
...
HRESULT CRenderObject::OnCreateDevice(IDirect3DDevice9 * pd3dDevice, TCHAR * FileName)
{
	...
	//装入颜色纹理
	szFile = GetResourceName(ENU_TEXBUMPMAP);
	...
	//装入凹凸纹理
	szFile = GetResourceName(ENU_TEXBUMPMAP);
	hr = D3DXCreateVolumeTextureFromFile(pd3dDevice, szFile, &m_pBumpTexture);
	if (FAILED(hr))
	{
		return hr;
	}
...
HRESULT CRenderObject::OnResetDevice()
{
	...
	D3DXHANDLE bumpTextureHandle = m_pEffect->GetParameterByName(0, "g_BumpTexture");
	hr = m_pEffect->SetTexture(bumpTextureHandle, m_pBumpTexture);
	if (FAILED(hr))
	{
		return hr;
	}
