...
float4 RenderScenePS(VS_OUTPUT In ) : COLOR0
{
   // 环境光
    ...
    //float4 allColor1 =  tex2D(MeshTextureSampler, In.TextureUV) *(fAmbient + v4Diffuse_1 + v4Diffuse_2 + v4fSpecular_1 + v4fSpecular_2);
	float4 allColor1 = v4fSpecular_1 + v4fSpecular_2;
	
	 float vtxf_a11 = frac(g_fTime / 120.0) * 3.14159265 * 2.0;
     float vtxf_a12 = In.z / 60.0 + sin(vtxf_a11) * 0.1;
     allColor1 *= float4(vtxf_a12, vtxf_a12, vtxf_a12, 1.0);

     float vtxf_a13 = frac(g_fTime / 360.0);
     float vtxf_h = clamp(In.z / 300.0, 0.0, 1.0);
     vtxf_a13 = abs(vtxf_a13 - 0.5) * 2.0;
     float vtxf_diff = step(0.005, abs(vtxf_h - vtxf_a13));
     allColor1.rgb += allColor1.rgb * (1.0 - vtxf_diff);
