...
float4 RenderScenePS(VS_OUTPUT In ) : COLOR0
{
	...
	if(dot(fDirectInVertex, normalize(g_vlightDirect ) ) > g_fCone)
	{
		//漫反射光
		fDiffuse =  0.4* dot(In.Normal, normalize(In.vLight)) / fLength;
		//高光
		fSpecular = pow(saturate(dot(reflect(normalize(In.vLight),In.Normal),In.vLookAt)), 10) / fLength;
	}
