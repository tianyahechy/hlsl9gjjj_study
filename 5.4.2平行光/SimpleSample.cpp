...
void CALLBACK OnD3D9FrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
	...
	// 观察矩阵
	D3DXVECTOR3 vEyePt(25 * sin(fTime), 7, 25 * cos(fTime));
	...
	//设置平行光的方向
	D3DXVECTOR4 vLightDirection(-7.0f * sinf(fTime * 0.7), -5.0f, -7.0f * cosf(fTime * 0.7), 1.0f);
	//将灯光位置给受光物体
	g_RenderObject_Ground.setLightParament(&vLightDirection);
	//将灯光物体变换到光线方向
	D3DXVECTOR3 vSrc = D3DXVECTOR3(1, 0, 0);
	D3DXVECTOR3 vDest = D3DXVECTOR3(vLightDirection.x, vLightDirection.y, vLightDirection.z);
	D3DXMATRIXA16 matOut;
	D3DXVectorRotate(&matOut, vDest, vSrc);
	matOut._41 = 0.0;
	matOut._42 = 8.0f;
	matOut._43 = 0.0;
	g_RenderLight.OnFrameMove(pd3dDevice, &matOut, &g_matView, &g_matProject, fTime, &vViewPosition);
