...
void CALLBACK OnD3D9FrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
	...
	//观察位置
	D3DXVECTOR4 vViewPosition(vEyePt.x, vEyePt.y, vEyePt.z, 0.0);
	g_RenderObject_1.OnFrameMove(pd3dDevice, &g_matWorld, &g_matView, &g_matProject, fTime, &vViewPosition);
	//设置灯光位置给受光物体，同时给灯光物体
	D3DXVECTOR4 vLightPosition(7.0f * sinf(fTime * 0.7), 4.0f, 7.0f * cosf(fTime * 0.7), 1.0f);
	//将灯光位置给受光物体
	g_RenderObject_1.setLightParament(&vLightPosition);
	D3DXMatrixTranslation(&g_matWorld, vLightPosition.x, vLightPosition.y, vLightPosition.z);
	g_RenderObject_2.OnFrameMove(pd3dDevice, &g_matWorld, &g_matView, &g_matProject, fTime, &vViewPosition);
