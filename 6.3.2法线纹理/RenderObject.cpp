...
{
	D3DXHANDLE lightPosiontHandle = m_pEffect->GetParameterByName(0, "g_vLightPosition");
	m_pEffect->SetVector(lightPosiontHandle, vPosition);
}
...
HRESULT CRenderObject::OnResetDevice()
{
	...
	D3DXHANDLE normalTextureHandle = m_pEffect->GetParameterByName(0, "g_NormalTexture");
	hr = m_pEffect->SetTexture(normalTextureHandle, m_pNormalTexture);
	if (FAILED(hr))
	{
		return hr;
	}

	D3DXHANDLE specularTextureHandle = m_pEffect->GetParameterByName(0, "g_SpecularTexture");
	hr = m_pEffect->SetTexture(specularTextureHandle, m_pSpecularTexture);
	return hr;
}
