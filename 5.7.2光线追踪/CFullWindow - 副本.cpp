...
HRESULT CFullWindow::OnResetDevice(IDirect3DDevice9 * pd3dDevice, D3DXVECTOR4 d3dViewPort)
{
	...
	D3DXHANDLE colorTextureHandle = m_pEffect->GetParameterByName(0, "texColorTexture");
	...
	m_pEffect->SetVector(viewportHandle, &d3dViewPort);
	return hr;

}

void CFullWindow::OnFrameMove(D3DXMATRIXA16 * pmat, float fTime)
{
	D3DXHANDLE timeHandle = m_pEffect->GetParameterByName(0, "g_fTime");
	m_pEffect->SetFloat(timeHandle, fTime);

	D3DXHANDLE viewHandle = m_pEffect->GetParameterByName(0, "g_matInvView");
	m_pEffect->SetMatrix(viewHandle, pmat);
}
