...
HRESULT CRenderObject::OnCreateDevice(IDirect3DDevice9 * pd3dDevice, TCHAR * FileName)
{
	...
	hr = D3DXLoadMeshFromX(szFile, D3DXMESH_SYSTEMMEM, pd3dDevice, NULL, &pBuffer, NULL, &m_dwMaterial, &pMeshSysMemClone);
	if (FAILED(hr))
	{
		return hr;
	}
	//处理材质
	m_pMaterial = new D3DXMATERIAL[m_dwMaterial];
	if (NULL == m_pMaterial)
	{
		return E_FAIL;
	}
	memcpy(m_pMaterial, pBuffer->GetBufferPointer(), sizeof(D3DXMATERIAL)* m_dwMaterial);
...
HRESULT CRenderObject::OnResetDevice(IDirect3DDevice9 * pd3dDevice)
{
	...
	D3DXHANDLE materialHandle = m_pEffect->GetParameterByName(0, "g_Material");
	hr = m_pEffect->SetValue(materialHandle, m_pMaterial, sizeof(D3DXMATERIAL));
	if (FAILED(hr))
	{
		return hr;
	}
