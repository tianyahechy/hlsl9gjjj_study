...
CShadowMapObject::CShadowMapObject()
{
	...
	m_TextureSizeX = 512;
	m_TextureSizeY = 512;
	m_pRTTTexturePosition = NULL;
	m_pRTTTextureNormal = NULL;
...
CShadowMapObject::~CShadowMapObject()
{
	...
	SAFE_RELEASE(m_pRTTTexturePosition);
	SAFE_RELEASE(m_pRTTTextureNormal);
...
HRESULT CShadowMapObject:: createRTT(IDirect3DDevice9 * pd3dDevice)
{
	HRESULT hr = S_OK;
	//创建shadowmap纹理
	hr = pd3dDevice->CreateTexture(m_TextureSizeX, m_TextureSizeY, 1, D3DUSAGE_RENDERTARGET, D3DFMT_A16B16G16R16F, D3DPOOL_DEFAULT, &m_pRTTTexturePosition, NULL);
	hr = pd3dDevice->CreateTexture(m_TextureSizeX, m_TextureSizeY, 1, D3DUSAGE_RENDERTARGET, D3DFMT_A16B16G16R16F, D3DPOOL_DEFAULT, &m_pRTTTextureNormal, NULL);
	hr = pd3dDevice->CreateTexture(m_TextureSizeX, m_TextureSizeY, 1, D3DUSAGE_RENDERTARGET, D3DFMT_A16B16G16R16F, D3DPOOL_DEFAULT, &m_pRTTTextureDepth, NULL);
	//创建深度模板
	...
	//设置纹理
	D3DXHANDLE textureHandle = m_pEffect->GetParameterByName(0, "g_ColorTexture");
	m_pEffect->SetTexture(textureHandle, m_pColorTexture);
	return hr;
...
{	
	HRESULT hr;
	//计算3个矩阵
	D3DXHANDLE RTTTechHandle = m_pEffect->GetTechniqueByName("RenderMTRTech");
	...
	//取三个纹理的页面
	IDirect3DSurface9* pRenderTargetPosition = NULL;
	hr = m_pRTTTexturePosition->GetSurfaceLevel(0, &pRenderTargetPosition);
	IDirect3DSurface9* pRenderTargetNormal = NULL;
	hr = m_pRTTTextureNormal->GetSurfaceLevel(0, &pRenderTargetNormal);
	IDirect3DSurface9* pRenderTargetDepth = NULL;
	hr = m_pRTTTextureDepth->GetSurfaceLevel(0, &pRenderTargetDepth);
	//设置到各个指定值
	hr = pd3dDevice->SetRenderTarget(0, pRenderTargetPosition);
	hr = pd3dDevice->SetRenderTarget(1, pRenderTargetNormal);
	hr = pd3dDevice->SetRenderTarget(2, pRenderTargetDepth);
	...
	pRenderTargetPosition->Release();
	pRenderTargetNormal->Release();
	pRenderTargetDepth->Release();

	//释放前面的渲染页面
	hr = pd3dDevice->SetRenderTarget(0, NULL);
	hr = pd3dDevice->SetRenderTarget(1, NULL);
	hr = pd3dDevice->SetRenderTarget(2, NULL);
...
void CShadowMapObject::setTextureSize(int sizeX, int sizeY)
{
	m_TextureSizeX = sizeX;
	m_TextureSizeY = sizeY;
}

void CShadowMapObject::OnFrameMove(D3DXMATRIXA16 * pWorld, D3DXMATRIXA16 * pView, D3DXMATRIXA16 * pProject, double fTime, D3DXVECTOR4 * pvEye)
{
	HRESULT hr = S_OK;
