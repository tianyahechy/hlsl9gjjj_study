...
HRESULT CRenderObject::OnResetDevice(IDirect3DDevice9 * pd3dDevice, const D3DSURFACE_DESC * pBackSurfaceDesc)
{
	HRESULT hr = S_OK;
	...
	//渲染到纹理
	hr = D3DXCreateTexture(pd3dDevice, pBackSurfaceDesc->Width, pBackSurfaceDesc->Height, 1, D3DUSAGE_RENDERTARGET, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &m_pRTTTexture);
	if (FAILED(hr))
	{
		hr = D3DXCreateTexture(pd3dDevice, pBackSurfaceDesc->Width, pBackSurfaceDesc->Height, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &m_pRTTTexture);
		if (FAILED(hr))
		{
			return hr;
		}
	...
	hr = D3DXCreateRenderToSurface(pd3dDevice, desc.Width, desc.Height, desc.Format, TRUE, D3DFMT_D16, &m_pRTTInterface);
	if (FAILED(hr))
	{
		return hr;
	}
