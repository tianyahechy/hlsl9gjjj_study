...
HRESULT CRenderObject::OnCreateDevice(IDirect3DDevice9 * pd3dDevice, TCHAR * FileName)
{
	...
	hr = D3DXLoadMeshFromX(szFile, D3DXMESH_SYSTEMMEM, pd3dDevice, NULL, &pMaterialBuffer, NULL, NULL, &pMeshSysMemClone);
	if (FAILED(hr))
	{
		return hr;
	}
	...
	//只计算一个材质
	memcpy(&m_pMaterial, pMaterialBuffer->GetBufferPointer(), sizeof(D3DXMATERIAL));
...
void CRenderObject::OnFrameMove(IDirect3DDevice9* pd3dDevice,D3DXMATRIXA16 * pWorld, D3DXMATRIXA16 * pView, D3DXMATRIXA16 * pProject, double fTime, D3DXVECTOR4* pvEye)
{
	...
	//设置材质属性，有几个属性不传递到效果文件
	D3DXMATRIXA16 matMaterial;
	matMaterial._11 = m_pMaterial.MatD3D.Ambient.r;
	matMaterial._12 = m_pMaterial.MatD3D.Ambient.g;
	matMaterial._13 = m_pMaterial.MatD3D.Ambient.b;
	matMaterial._14 = m_pMaterial.MatD3D.Ambient.a;

	matMaterial._21 = m_pMaterial.MatD3D.Diffuse.r;
	matMaterial._22 = m_pMaterial.MatD3D.Diffuse.g;
	matMaterial._23 = m_pMaterial.MatD3D.Diffuse.b;
	matMaterial._24 = m_pMaterial.MatD3D.Diffuse.a;

	matMaterial._31 = m_pMaterial.MatD3D.Specular.r;
	matMaterial._32 = m_pMaterial.MatD3D.Specular.g;
	matMaterial._33 = m_pMaterial.MatD3D.Specular.b;
	matMaterial._34 = m_pMaterial.MatD3D.Specular.a;

	D3DXHANDLE materialHandle = m_pEffect->GetParameterByName(0, "g_matMaterial");
	m_pEffect->SetMatrix(materialHandle, &matMaterial);
	float Determinant;
	D3DXMATRIX matOut;
